...
if __name__ == "__main__":
	...
	## add empty list for graph 訓練に対してのPSNRをグラフにする
	loss_history = []
		...
		with tqdm(desc="Training", total=n_steps, unit="steps") as t:
			while testbed.frame():
				...
				if now - tqdm_last_update > 0.1:
					...
					loss_history.append(testbed.loss)
					# print(testbed.loss)
					old_training_step = testbed.training_step
					tqdm_last_update = now
				## add エポックごとのLossを保存するため
				else:
					loss_history.append(testbed.loss)

	##add
	# グラフの作成
	if args.test_transforms == None:
		...
		# グラフの保存
		save_path = os.path.join(os.getcwd(), 'training_epochs_loss.png')
